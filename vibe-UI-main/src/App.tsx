...
import TestShadcn from './components/ui/TestShadcn';
...
const TABS = [
  ...
  { id: 'storytelling', label: 'Data Storytelling' },
  { id: 'test', label: 'Test Components' },
];
  ...
  return (
    ...
    <div className={`${styles.appContainer} ${theme === 'dark' ? styles.darkTheme : ''}`}>
      ...
      <main className={styles.scrollableContent}>
            ...
            {activeTab === 'storytelling' && <StorytellingView />}
            {activeTab === 'test' && <TestShadcn />}
          </main>
        </div>
        </MetricsProvider>
      </ChatProvider>
